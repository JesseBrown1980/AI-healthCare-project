...
async def test_history_limit_keeps_recent_entries():
    ...
    assert [entry["patient_id"] for entry in history] == ["p1", "p1"]
    assert [entry["analysis_timestamp"] for entry in history] == [t2, t3]
...
def test_clear_history_empties_cache():
    ...
    # Populate history via public method which delegates to service
    analyzer.history_service.analysis_history = {"p1": [{"patient_id": "p1"}], "p2": [{"patient_id": "p2"}]}
