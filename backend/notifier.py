...
class Notifier:
    ...
    async def notify(
        ...
        if self.agent_manager:
            review_decision = await self.agent_manager.review_notification(payload)
            if review_decision.get("action") == "REJECT":
                logger.warning(
                    f"Notification REJECTED by AgentManager. Reason: {review_decision.get('reason')}"
                )
                return {"status": "rejected", "reason": review_decision.get("reason")}
            
            # If approved, we proceed, but we might log strictly
            if review_decision.get("reason"):
                logger.info(f"Notification APPROVED by AgentManager: {review_decision.get('reason')}")
