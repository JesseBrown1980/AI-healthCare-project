...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    payload = create_error_response(
        ...
        hint=get_hint_for_status_code(500),
        detail=error_detail,
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    # Extract message from detail (may include hint in parentheses)
    message = exc.detail if exc.detail else "Request failed"
    if " (" in message and message.endswith(")"):
        # Hint already included in detail, extract it
        parts = message.rsplit(" (", 1)
        message = parts[0]
        hint = parts[1].rstrip(")")
    else:
        hint = get_hint_for_status_code(exc.status_code)
