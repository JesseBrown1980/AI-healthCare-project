...
async def init_database() -> None:
    ...
    if "sqlite" not in db_url.lower():
        ...
        # SQLite needs specific handling
        if ":memory:" in db_url:
            pool_settings = {
    ...
    try:
        if ":memory:" in db_url:
            # For in-memory database, we MUST use the same engine/connection
            # Alembic would create a separate sync engine with a separate in-memory DB
            raise RuntimeError("In-memory database requires direct create_all")
...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    Works with both FastAPI Depends and direct 'async with' usage.
    """
...
# Alias for backwards compatibility (same as get_db_session now)
