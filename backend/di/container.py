...
class ServiceContainer:
    ...
    async def startup(self) -> None:
        if os.getenv("TESTING", "false").lower() == "true":
            logger.info("TESTING mode enabled: Skipping heavy ML model initialization")
            # Initialize light services only needed for basic API tests
            self.fhir_client = FhirHttpClient(server_url="http://mock-fhir")
            ...
            self.analysis_job_manager = AnalysisJobManager(ttl_seconds=self.analysis_cache_ttl_seconds)
            self.patient_analyzer = None
            self.llm_engine = None
            self.rag_fusion = None
            self.s_lora_manager = None
            self.mlc_learning = None
            self.aot_reasoner = None
            return
        ...
        self.agent_manager = AgentManager(llm_engine=self.llm_engine)

        logger.info("Loading Notifier...")
        self.notifier = Notifier(agent_manager=self.agent_manager)
